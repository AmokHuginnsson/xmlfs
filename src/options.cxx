...
---       `xmlfs' 0.0.0 (c) 1978 by Marcin 'Amok' Konarski         ---
	...
	options.cxx - this file is integral part of `xmlfs' project.
...
bool set_variables( HString& option_, HString& value_ ) {
	static bool const XMLFS_RC_DEBUG( !! ::getenv( "XMLFS_RC_DEBUG" ) );
	if ( XMLFS_RC_DEBUG ) {
...
int handle_program_options( int argc_, char** argv_ ) {
	...
	bool stop = false;
	po( "fs-file", program_options_helper::option_value( setup._fsFilePath ), 'F', HProgramOptionsHandler::OOption::TYPE::REQUIRED, "path to file system image" )
		( "log_path", program_options_helper::option_value( setup._logPath ), HProgramOptionsHandler::OOption::TYPE::REQUIRED, "path pointing to file for application logs", "path" )
		...
		( "debug", program_options_helper::option_value( setup._debug ), 'd', HProgramOptionsHandler::OOption::TYPE::NONE, "put additional debugging information into the logs" );
	po.process_rc_file( "xmlfs", "", set_variables );
	if ( setup._logPath.is_empty() )
		setup._logPath = "xmlfs.log";
	int unknown( 0 );
